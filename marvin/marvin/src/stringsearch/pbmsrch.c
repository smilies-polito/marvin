...
int search_wrap(int golden)
{

      char *here;
      char *find_strings[] = {"abb",  "you", "not", "it", "dad", "yoo", "hoo",
                              "oo", "oh", "xx", "x", "field", "new", "row",
      ...
      if(golden > 0){
		  ...
		  index_string = rand() % sizeof(find_strings)/sizeof(char*);
		  pointer_input[0] = index_string;

      }else{
    	  index_string = pointer_input[0];
      }
      //xil_printf("index string: %d\n\n", index_string);
      int i;


	  init_search(find_strings[index_string]);
	  here = strsearch(search_strings[index_string]);
	  //xil_printf("\"%s\" is%s in \"%s\"", find_strings[index_string],here ? "" : " not", search_strings[index_string]);
	  //if (here)
		  //xil_printf(" [\"%s\"]", here);
	  putchar('\n');
	 ...
	 if(golden > 0){
		int i = 0;
		while(here[i] != '\0'){
			pointer_gold[i] = here[i];
			i++;
		}
		pointer_gold[i] = '\0';
	 }else{
		char golden_here[256];
		int i = 0;
		while(pointer_gold[i] != '\0'){
			golden_here[i] = pointer_gold[i];
			i++;
		}
		golden_here[i] = '\0';
		//xil_printf("gold: %s\n", golden_path);

		int result = strcmp(here, golden_here);
		if(result != 0){
			corr = 0;
		}
