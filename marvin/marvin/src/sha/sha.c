...
int sha_print(SHA_INFO *sha_info, int golden)
{
	int corr = 1;
	if(golden > 0){
		int* pointer_gold = (int*)0x40000100;
		//xil_printf("Result: %x - %x - %x - %x - %x\n", sha_info->digest[0], sha_info->digest[1], sha_info->digest[2],sha_info->digest[3], sha_info->digest[4]);

		pointer_gold[0] = sha_info->digest[0];
		pointer_gold[4] = sha_info->digest[1];
		pointer_gold[8] = sha_info->digest[2];
		pointer_gold[12] = sha_info->digest[3];
		pointer_gold[16] = sha_info->digest[4];


	}else{
		int* pointer_gold = (int*)0x40000100;
		//xil_printf("Result: %x - %x - %x - %x - %x\n", sha_info->digest[0], sha_info->digest[1], sha_info->digest[2], sha_info->digest[3], sha_info->digest[4]);
		if(sha_info->digest[0] != pointer_gold[0] || sha_info->digest[1] != pointer_gold[4] || sha_info->digest[2] != pointer_gold[8] || sha_info->digest[3] != pointer_gold[12] || sha_info->digest[4] != pointer_gold[16]){
			corr = 0;
		}
